...
def stub_task(view)
  if view == "completed"
    assigns[:task] = FactoryBot.create(:task, completed_at: Time.now - 1.minute)
  elsif view == "assigned"
    assigns[:task] = FactoryBot.create(:task, assignee: FactoryBot.create(:user))
  else
    assigns[:task] = FactoryBot.create(:task)
  end
end
